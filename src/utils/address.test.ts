...
describe("normalizeStarknetAddress", () => {
  it("normalizes missing prefixes, mixed case, whitespace, and short values", () => {
    ...
    expect(normalizeStarknetAddress("  0XABC  ")).toBe(
      `0x${"0".repeat(61)}abc`,
    );
  ...
  it("matches the previous route variants after choosing the canonical trimmed form", () => {
    ...
    expect(normalizeStarknetAddress(alreadyCanonical)).toBe(
      routeVariant(alreadyCanonical),
    );
    expect(normalizeStarknetAddress("0x0001")).toBe(
      transactionsVariant("0x0001"),
    );
    expect(normalizeStarknetAddress("0x0001")).toBe(
      normalizeStarknetAddress("0x1"),
    );
  });
  ...
  it("rejects empty, non-hex, and wider-than-felt values", () => {
    ...
    expect(() => normalizeStarknetAddress(`0x1${"0".repeat(64)}`)).toThrow(
      /exceeds/,
    );
  });
});
