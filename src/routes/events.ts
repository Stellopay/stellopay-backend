...
/**
 * Processes Starknet transaction receipt and stores all emitted events.
 * Decodes events using WorkAgreement or PayrollEscrow ABIs, or falls back to heuristics.
 *
 * @param txHash - The Starknet transaction hash to process.
 * @returns An object containing processing message, processed event details, and the normalized transaction hash.
 * @throws Error if the transaction hash is invalid or the transaction cannot be retrieved.
 */
export async function processTxEvents(txHash: string) {
  // Validate tx_hash format before processing to prevent invalid inputs/SSRF
  const txHashRegex = /^0x?[0-9a-fA-F]{1,64}$/;
  if (!txHash || !txHashRegex.test(txHash) || txHash.length > 66) {
    throw new Error("Invalid Starknet transaction hash format");
  }
...
eventsRouter.post("/events/process_tx/:tx_hash", async (req, res, next) => {
  try {
    ...
    const result = await processTxEvents(tx_hash);
    res.json(result);
  } catch (e: any) {
...
eventsRouter.post("/events/process_batch", async (req, res, next) => {
  try {
    ...
    for (const txHash of tx_hashes) {
      try {
        const result = await processTxEvents(txHash);
        results.push({
          txHash: result.transactionHash,
          status: "processed",
          eventCount: result.eventsProcessed.length
        });
      } catch (e) {
