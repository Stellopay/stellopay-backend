...
const { envMock, dbMock, schemaMock } = vi.hoisted(() => {
  /**
   * Minimal drizzle-like query chain that is both awaitable and chainable.
   * Each factory call produces an independent chain instance.
   */
...
// ---------------------------------------------------------------------------
// App factory — fresh instance per test so state never leaks
// ---------------------------------------------------------------------------
...
/**
 * Sequences db.select return values: call N returns sequences[N-1].
 * Use this for endpoints that issue multiple select calls sequentially.
 */
...
/**
 * Makes every db.select throw err synchronously (simulates a pool/connection error
...
describe("BILLING_ENABLED feature flag", () => {
  ...
  it("passes through to handlers when the flag is true", async () => {
    envMock.BILLING_ENABLED = true;
    // Profile lookup only — paymentMethods and invoices default to []
...
describe("profileId validation", () => {
  it("rejects a profileId longer than 128 chars with 400", async () => {
    ...
    const res = await request(makeApp()).get(`/api/v1/billing/profiles/${longId}/summary`);
  ...
  it("accepts alphanumeric-and-dash profileIds", async () => {
    ...
    // Any non-400 response proves the validator passed
    expect(res.status).not.toBe(400);
  });
});

// ===========================================================================
// 3. GET /billing/profiles/:profileId  (full profile)
// ===========================================================================

describe("GET /billing/profiles/:profileId", () => {
  it("returns 200 with profile, paymentMethods, and invoices on success", async () => {
    // 1st select → profile row; 2nd & 3rd → Promise.all(paymentMethods, invoices)
  ...
  it("never exposes taxId in the profile response", async () => {
    mockSelectSequence([BASE_PROFILE], [], []);
    const res = await request(makeApp()).get(`/api/v1/billing/profiles/${PROFILE_ID}`);
    expect(res.status).toBe(200);
  ...
  it("never exposes dateOfBirth in the profile response", async () => {
    mockSelectSequence([BASE_PROFILE], [], []);
    const res = await request(makeApp()).get(`/api/v1/billing/profiles/${PROFILE_ID}`);
    expect(res.status).toBe(200);
  ...
  it("returns 500 and does not leak internal error details on db throw", async () => {
...
describe("GET /billing/profiles/:profileId/general-information", () => {
  it("returns 200 with safe profile fields and a computed fullAddress", async () => {
    mockSelectSequence([BASE_PROFILE]);
  ...
  it("builds fullAddress from only the present address parts", async () => {
    const partial = {
      ...BASE_PROFILE,
      street: null,
      city: "London",
  ...
  it("returns 404 for an unknown profileId", async () => {
    mockSelectSequence([]);
    const res = await request(makeApp()).get(
      `/api/v1/billing/profiles/ghost/general-information`,
    );
    expect(res.status).toBe(404);
  });
  ...
  it("returns 500 on db error without leaking details", async () => {
    ...
    expect(JSON.stringify(res.body)).not.toContain("db fail");
  });
});
...
describe("GET /billing/profiles/:profileId/payment-methods", () => {
  ...
  it("returns an empty array when the profile has no payment methods", async () => {
  ...
  it("returns 404 when the profile does not exist", async () => {
    mockSelectSequence([]);
    const res = await request(makeApp()).get(
      `/api/v1/billing/profiles/no-such-id/payment-methods`,
    );
    expect(res.status).toBe(404);
  });
...
describe("GET /billing/profiles/:profileId/invoices", () => {
  ...
  it("returns an empty invoices array when none exist", async () => {
    mockSelectSequence([{ id: PROFILE_ID }], []);
    const res = await request(makeApp()).get(`/api/v1/billing/profiles/${PROFILE_ID}/invoices`);
    expect(res.status).toBe(200);
...
// ===========================================================================
// 7. GET /billing/profiles/:profileId/summary  — billing math (HTTP layer)
// ===========================================================================

describe("GET /billing/profiles/:profileId/summary – billing math", () => {
  function summaryRow(override: {
    annualRewardLimit?: string;
    usedAmount?: string;
  }) {
    return {
      id: PROFILE_ID,
      profileType: BASE_PROFILE.profileType,
      annualRewardLimit: override.annualRewardLimit ?? BASE_PROFILE.annualRewardLimit,
      usedAmount: override.usedAmount ?? BASE_PROFILE.usedAmount,
      currency: BASE_PROFILE.currency,
    };
  }
  ...
  it("returns correct remainingAmount = limit − used", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body.data.annualRewardLimit).toBe(1000);
    expect(res.body.data.usedAmount).toBe(250);
    expect(res.body.data.remainingAmount).toBe(750);
  });

  it("clamps remainingAmount to 0 when usedAmount exceeds the limit (over-limit)", async () => {
  ...
  it("remainingAmount is 0 when usedAmount exactly equals the limit", async () => {
  ...
  it("progressPercentage is 0 when annualRewardLimit is 0 (no divide-by-zero)", async () => {
  ...
  it("progressPercentage can exceed 100 when over-limit (not clamped)", async () => {
    mockSelectSequence([summaryRow({ annualRewardLimit: "500.000000", usedAmount: "600.000000" })]);
    const res = await request(makeApp()).get(`/api/v1/billing/profiles/${PROFILE_ID}/summary`);
    expect(res.status).toBe(200);
  ...
  it("returns all required summary fields", async () => {
    ...
    expect(d).toHaveProperty("profileId");
    expect(d).toHaveProperty("profileType");
    expect(d).toHaveProperty("annualRewardLimit");
    expect(d).toHaveProperty("usedAmount");
    expect(d).toHaveProperty("remainingAmount");
    expect(d).toHaveProperty("currency");
    expect(d).toHaveProperty("progressPercentage");
  });
  ...
  it("returns 500 on db error without leaking internal details", async () => {
...
// ===========================================================================
// 9. Pure helper unit tests (no HTTP, no mocks needed)
// ===========================================================================
...
describe("computeBillingSummary (pure unit tests)", () => {
  ...
  it("over-limit: remainingAmount clamped to 0, progressPercentage > 100", () => {
    const r = computeBillingSummary("500.000000", "600.000000");
    expect(r.remainingAmount).toBe(0);
    expect(r.progressPercentage).toBe(120);
  });

  it("exact limit: remainingAmount=0, progressPercentage=100", () => {
    const r = computeBillingSummary("1000.000000", "1000.000000");
    expect(r.remainingAmount).toBe(0);
    expect(r.progressPercentage).toBe(100);
  });

  it("rounds progressPercentage to 2 decimal places", () => {
    const r = computeBillingSummary("3.000000", "1.000000");
    expect(r.progressPercentage).toBe(33.33);
  });
});

describe("buildFullAddress (pure unit tests)", () => {
  it("joins all present parts in order: street, city, state, zip, country", () => {
    expect(buildFullAddress("123 Main St", "Springfield", "IL", "62701", "US")).toBe(
  ...
  it("returns single part as-is (no trailing comma)", () => {
